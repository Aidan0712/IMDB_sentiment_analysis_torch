...
class Capsule(nn.Module):
    def __init__(self, num_hiddens, bidirectional, num_capsule=5, dim_capsule=5, routings=4, **kwargs):
        ...
        self.dim_capsule = self.num_capsule * 2
    ...
    def forward(self, inputs):
        ...
        u_hat_vecs = torch.matmul(inputs, self.W)  # (batch, input_num_capsule, num_capsule * hidden_dim * 2)
        batch_size = inputs.size(0)
        input_num_capsule = inputs.size(1)

        # ✅ 正确 reshape：不要用 dim_capsule，用 num_hiddens * 2
        u_hat_vecs = u_hat_vecs.view(batch_size, input_num_capsule, self.num_capsule, self.num_hiddens * 2)
        print("u_hat_vecs after view:", u_hat_vecs.shape)
...
class SentimentNet(nn.Module):
    def __init__(self, embed_size, num_hiddens, num_layers, bidirectional, weight, labels, use_gpu, **kwargs):
        ...
        self.num_capsule = 5
        self.decoder = nn.Linear(self.num_capsule * self.num_hiddens * 2, labels)
        # if self.bidirectional:
    ...
    def forward(self, inputs):
        ...
        states, hidden = self.encoder(embeddings.permute(1, 0, 2))  # (seq_len, batch_size, hidden*direction)
        states = states.permute(1, 0, 2)  # (batch_size, seq_len, hidden*direction)
        print(states.shape)  # 调试时保留
        capsule = self.capsule(states)
        encoding = capsule.view(capsule.size(0), -1)  # 展平所有 capsule
